"""depricated so dont use it anyone"""
...
async def upload_reading(
    ...
    if patient.get("OwnerId") != current_user.get("OwnerId"):
    ...
    public_id = f"smart-glove/{OwnerId}/{meta.PatientId}/{uuid.uuid4()}"
    try:
    ...
    hb = None
    if meta.HemoglobinLevel is not None:
        try:
            hb = float(meta.HemoglobinLevel)
        except Exception:
            hb = None
    if hb is None:
        try:
            prediction = await predict_hemoglobin_from_image(processed_bytes)
            hb = float(prediction.HemoglobinLevel)
    ...
    anemic_flag = is_anemic(hb, patient_age, patient_gender, meta.IsPregnant)
    ...
    return {
        ...
        "reading_id": reading_doc["ReadingId"],
